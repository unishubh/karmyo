...
.factory('NSModelFactory', ['$log', function($log){
  ...
  Store.prototype = {
    ...
    create: function(model) {
      if (!model._id && model._id !== 0) {
        model._id = guid();
        model.set(model.idAttribute, model._id);
      }
      this.localStorage().setItem(this._itemName(model._id), this.serializer.serialize(model));
    ...
    find: function(model) {
      return this.serializer.deserialize(this.localStorage().getItem(this._itemName(model._id)));
    },
    ...
    destroy: function(model) {
      this.localStorage().removeItem(this._itemName(model._id));
      var modelId = model._id.toString();
